...
# Merge sample-qc list and pset-qc list into finher_expr_norm list
# Use as supplementary file in GEO
# ! Rename list to reflect list contents

# original [1] "sum"    "pvac"   "maxvar" "combat"
nme = c("Normalized_expression", "PVAC_pset_filtering", "MaxVar_pset_after_PVAC", "Combat_after_MaxVar")

names(finher_expr_norm1$rma) <- nme
names(finher_expr_norm1$mas) <- nme
names(finher_expr_norm2$rma) <- nme
names(finher_expr_norm2$mas) <- nme

# Include a README file containing column descriptions in the merged list
readme <- read_excel(path = "results/tables/GEO_preprocessed_README.xlsx") # manually created

dim(celmap) # 337 3


rma_all_samples_only_pset_filtering <- c(list(README = readme),
                                         finher_expr_norm1$rma,
                                         list(Sample_qc = sample_qc_norm1$rma,
                                              Probeset_qc = pset_qc_norm1$rma))
mas_all_samples_only_pset_filtering <- c(list(README = readme),
                                         finher_expr_norm1$mas,
                                         list(Sample_qc = sample_qc_norm1$mas,
                                              Probeset_qc = pset_qc_norm1$mas))

rma_sample_and_pset_filtered <- c(list(README = readme),
                                  finher_expr_norm2$rma,
                                  list(Sample_qc = sample_qc_norm2$rma,
                                       Probeset_qc = pset_qc_norm2$rma))
mas_sample_and_pset_filtered <- c(list(README = readme),
                                  finher_expr_norm2$mas,
                                  list(Sample_qc = sample_qc_norm2$mas,
                                       Probeset_qc = pset_qc_norm2$mas))


# Data matrix and metadata for GEO
geo_data_matrix = mas_all_samples_only_pset_filtering$Normalized_expression
geo_metadata = mas_all_samples_only_pset_filtering$Sample_qc %>%
...
# 4. Write out xlsx files of MAS normalized full data matrix, qc metrics, and
#    MAS/RMA normalized filtered matrices
# ==============================================================================


write_xlsx(
  x = geo_data_matrix,
  path = str_c(out_tables,"GEO_reporcessed_data_matrix.xlsx"),
...
write_xlsx(
  x = geo_metadata,
  path = str_c(out_tables,"GEO_reporcessed_metadata_sampleqc.xlsx"),
  format_headers = FALSE,
  use_zip64 = FALSE
)

write_xlsx(
  x = rma_all_samples_only_pset_filtering,
  path = str_c(out_tables,"GEO_reporcessed_RMA_all_samples_only_pset_filtering.xlsx"),
  format_headers = FALSE,
  use_zip64 = FALSE
)

write_xlsx(
  x = mas_all_samples_only_pset_filtering,
  path = str_c(out_tables,"GEO_reporcessed_MAS_all_samples_only_pset_filtering.xlsx"),
  format_headers = FALSE,
  use_zip64 = FALSE
)

write_xlsx(
  x = rma_sample_and_pset_filtered,
  path = str_c(out_tables,"GEO_reporcessed_RMA_sample_and_pset_filtered.xlsx"),
  format_headers = FALSE,
  use_zip64 = FALSE
)

write_xlsx(
  x = mas_sample_and_pset_filtered,
  path = str_c(out_tables,"GEO_reporcessed_MAS_sample_and_pset_filtered.xlsx"),
  format_headers = FALSE,
  use_zip64 = FALSE
)
